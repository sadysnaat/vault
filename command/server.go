...
type ServerCommand struct {
	*BaseCommand
	...
	flagDevLeasedGeneric bool
	flagDevThreeNode     bool
	flagTestVerifyOnly   bool
}

func (c *ServerCommand) Synopsis() string {
	return "Starts a Vault server"
}
...
func (c *ServerCommand) Run(args []string) int {
	...
	if c.flagDevHA || c.flagDevTransactional || c.flagDevLeasedGeneric || c.flagDevThreeNode {
		c.flagDev = true
	}
	...
	if !c.flagDev {
		...
		case c.flagDevRootTokenID != "":
			c.UI.Error("Cannot specify toot token ID outside \"dev\" mode")
			return 1
		}
	}
...
CLUSTER_SYNTHESIS_COMPLETE:
	...
	if c.flagDev {
		...
		c.UI.Warn("")
		c.UI.Warn(wrapAtLength(
			"The unseal key and initial root token are displayed below in case " +
				"you want to seal/unseal the Vault or re-authenticate."))
		c.UI.Warn("")
